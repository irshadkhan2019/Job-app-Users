...
import { BadRequestError, IBuyerDocument, IEducation, IExperience, ISellerDocument } from '@irshadkhan2019/job-app-shared';
...
const seed = async (req: Request, res: Response): Promise<void> => {
  ...
  const { count } = req.params; //no of sellers needed

  const buyers: IBuyerDocument[] = await getRandomBuyers(parseInt(count, 10));
  // These buyers will become sellers
  for(let i = 0; i < buyers.length; i++) {
    const buyer: IBuyerDocument = buyers[i];
    // check if the current buyer is already seller
    ...
    const skills: string[] = ['Programming', 'Web development', 'Mobile development', 'Proof reading', 'UI/UX', 'Data Science', 'Financial modeling', 'Data analysis','Game development' ];
    ...
    const seller: ISellerDocument = {
      ...
      description: basicDescription.length <= 150 ? basicDescription : basicDescription.slice(0, 150),
      ...
      socialLinks: ['https://www.unrealengine.com', 'http://youtube.com', 'https://facebook.com'],
      certificates: [
        {
